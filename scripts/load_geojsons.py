...
def counties():
    filename = 'static/reduced-mapshaper-counties.geojson'
    geographies = GeographicArea.objects.all()
    current_geographies = []
    ...
    with open(filename, 'r') as f:
        ...
        for feature in data['features']:
            fips = feature['properties']['GEOID10']
            #check if it exists
            if (GeographicArea.objects.filter(fips=fips).count() > 0):
                county = GeographicArea.objects.get(fips=fips)
                state = county.state
                ...
                new_feature = {
                        ...
                        "properties": {
                            "Id": county.id,
                            "GeographyType": "geographic_area",
                            "FIPS": fips,
                            "State": state,
                            "County": county_name,
                        }
                    }
                county.geo_data=new_feature
                county.save()
def run():
    lrs()
    counties()
